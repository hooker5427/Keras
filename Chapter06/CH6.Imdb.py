from keras.datasets import imdb  # 导入数据集 情感分析练手数据集
...
XTrain= pad_sequences(XTrain, maxlen=100) # maxlen单词最大长度 ， 不够padding ,超过直接截断
...
from keras.layers.embeddings import Embedding       # embedding 数据低维度的压缩表示
...
RNNModel.add(SimpleRNN(32, input_shape=(10000, 100), return_sequences=False)) # return_sequences参数，：False 只是返回最后一个输出 True 返回真个序列的输出
...
RNNModel.fit(XTrain, YTrain, validation_data=(XTest, YTest), epochs=3, batch_size=64, verbose=1) # validation_data验证集
